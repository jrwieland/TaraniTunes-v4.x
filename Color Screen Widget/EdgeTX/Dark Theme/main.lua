...
local function refreshZoneTiny(tunes)
  ...
  lcd.drawText(tunes.zone.x, tunes.zone.y+15,"> ".. playlist[selection][2],YELLOW+SMLSIZE)
end
...
local function refreshZoneSmall(tunes)
  ...
  lcd.drawText(tunes.zone.x, tunes.zone.y+17, playlist[selection][1], YELLOW+SMLSIZE)
end 
...
local function refreshZoneMedium(tunes)
  ...
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(219,172,52))
  ...
  if selection == #playlist then
    ...
    lcd.drawText(tunes.zone.x, tunes.zone.y+60, string.char(126) .. playlist[selection][1],YELLOW+SMLSIZE)
  else
    if selection == 1 then	
      lcd.drawText(tunes.zone.x, tunes.zone.y+30, string.char(126) .. playlist[selection][1],YELLOW+SMLSIZE)
      lcd.drawText(tunes.zone.x, tunes.zone.y+45, playlist[selection + 1][1],SMLSIZE)
      ...
      lcd.drawText(tunes.zone.x, tunes.zone.y+45, string.char(126) .. playlist[selection][1], YELLOW+SMLSIZE)
...
local function refreshZoneLarge(tunes)
  ...
  if selection == #playlist then
    ...
    lcd.drawText(tunes.zone.x, tunes.zone.y+125, string.char(126) .. playlist[selection][1],YELLOW)
  else
    if selection == 1 then	
      lcd.drawText(tunes.zone.x, tunes.zone.y+85, string.char(126) .. playlist[selection][1],YELLOW)
      lcd.drawText(tunes.zone.x, tunes.zone.y+105, playlist[selection + 1][1])
      ...
      lcd.drawText(tunes.zone.x, tunes.zone.y+105, string.char(126) .. playlist[selection][1],YELLOW)
...
-- Flight Time
  ...
  lcd.drawTimer(380, 72, fly, 2+GREEN)	
  ...
  if battsw == -1024 then 
    ...
    if battsw == 0 then
      ...
      lcd.drawText(90,75,"HV 4.35", SMLSIZE)
    end 
  end
...
-- Cell Voltage
  ...
  lcd.drawChannel(380, 119,"Cel", PREC2+ORANGE) 
...
-- Tementry Separator Bar
  local y = 160
  lcd.drawFilledRectangle(40,y-18,400,6,RED)
...
--Music Display
  ...
  if selection == #playlist then
    ...
    lcd.drawText(40, y+56, string.char(126) .. playlist[selection][1],YELLOW)
  else
    if selection == 1 then	
      lcd.drawText(40, y+20, string.char(126) .. playlist[selection][1],YELLOW)
      lcd.drawText(40, y+38, playlist[selection + 1][1])
      lcd.drawText(40, y+56, playlist[selection + 2][1])
    else		
      lcd.drawText(40, y+20, playlist[selection - 1][1])
      lcd.drawText(40, y+38, string.char(126) .. playlist[selection][1],YELLOW)
...
--Transmitter battery
  ...
  local battsw = getValue('se')
  if battsw == -1024 then 
    lcd.drawText(50,77,"SHORT 4.2") --older Battery
    lcd.drawSwitch(10,77,13)else
    if battsw == 0 then
      lcd.drawSwitch(10,77,14)
      lcd.drawText(50,77,"REG 4.2") else
      lcd.drawSwitch(10,77,15)
      lcd.drawText(50,77,"HV 4.35")
    end 
  end
...
-- Speed
  ...
  lcd.drawChannel(420,83,"GSpd+",2+YELLOW)

-- Flight Time
  local fly=model.getTimer(1).value  --flight duration timer: 0=timer1, 1=timer2
  lcd.drawText(384, 4, "Flight Time")
  lcd.drawTimer(380, 22, fly, DBLSIZE+PREC2+GREEN)	

-- Cell Voltage
  lcd.drawText(387, 104, "Lipo",MIDSIZE)
  lcd.drawChannel(370, 130,"Cels", DBLSIZE+PREC2+ORANGE) 
...
-- Altitude from Altimeter ("Alt") or GPS Sensor ("GAlt")
  ...
  lcd.drawText(135, 143,"//Max")
  lcd.drawChannel(190,143,"GAlt+",2)  
...
--Tementry separator bar
  local y = 185
  lcd.drawFilledRectangle(6,y-18,462,6,RED)

-- Music Progress Bar 
  local upTime=model.getTimer(2).value --do not change this value it is how long the current song has played
  local k = math.floor((upTime/long)*40)*10
  lcd.drawTimer(7, y-10, upTime, SMLSIZE+BRIGHTGREEN)
  lcd.drawTimer(434,y-10, long, SMLSIZE+BRIGHTGREEN)
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(219,172,52))
  lcd.drawFilledRectangle(44,y-10,k,14,CUSTOM_COLOR)

-- Music Display
  local selection=playingSong
  lcd.drawText(150, y+4, "Selected Playlist >>  ".. title, SMLSIZE)
  if selection == #playlist then
    lcd.drawText(12, y+20, playlist[selection - 2][1])
    lcd.drawText(12, y+40, playlist[selection - 1][1])
    lcd.drawText(12, y+60, string.char(126) .. playlist[selection][1],YELLOW)
  else
    if selection == 1 then	
      lcd.drawText(12, y+20, string.char(126) .. playlist[selection][1],YELLOW)
      lcd.drawText(12, y+40, playlist[selection + 1][1])
      lcd.drawText(12, y+60, playlist[selection + 2][1])
    else		
      lcd.drawText(12, y+20, playlist[selection - 1][1])
      lcd.drawText(12, y+40, string.char(126) .. playlist[selection][1], YELLOW)
