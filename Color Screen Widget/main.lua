...
elogo = Bitmap.open("/THEMES/Darkblue/topmenu_opentx.bmp")
...
-- Zone size: top bar widget
local function refreshZoneTiny(tunes)
  local selection=playingSong
  lcd.drawText(tunes.zone.x, tunes.zone.y,"List # "..model.getGlobalVariable(8, 0), INVERS+SMLSIZE)
  lcd.drawText(tunes.zone.x, tunes.zone.y+15,"> ".. playlist[selection][2],INVERS+SMLSIZE)
end
...
local function refreshZoneMedium(tunes)
  ...
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(0,154,0))
...
--Flight Time
  ...
  lcd.drawTimer(380, 72, fly, 2)	
    ...
    --[[ Flight Battery Loaded    ### I use different capacity batteries the timers are
